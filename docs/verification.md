...
- Registry installation: published as 0.1.0 on dartpub.dev. An empty package cache resolves the hosted package, and fresh consumer apps build on iOS and Android. The registry-installed code completes Face ID authentication on iOS and device-PIN authentication on Android. The downloaded archive matches its registry SHA-256, and native/Dart implementation files match the release source.
